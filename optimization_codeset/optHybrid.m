...
if ~opt.highresobj
    ...
    disp('error do not use highresobj')
...
%% Sensitivity and Modifiers - obselete
...
if opt.singlepoint == 1
    ...
    output.min.dp,output.min.comp_plat_mass,output.min.S1, output.min.S2, output.min.Pdies, output.min.Pinso, output.min.Pwind, output.min.Pwave,output.min.Pcurr,...
...
%     [~,check_s] = simHybrid(GenCoords, opt.Smax_n/2,opt,data,atmo,batt,econ,uc,bc,dies,inso,wave,turb,cturb);
...
%% DIY Optimization
    ...
    while tol == false && tel_i <=opt.tel_max
        ...
        if all(opt.alg == 'tel') || all(opt.alg == 'to2')
            ...
            % if opt.pd == 6 - commenting out because this should only be used with 2D optimization
        ...
        % elseif all(opt.alg == 'per') - obselete optimization
        %     if tel_i == 1 %can't send output to function before output is defined
        ...
        %     n = opt.bf.n;
        %     o = opt.bf.o;
        % elseif all(opt.alg == 'p2t') - obselete optimization
        %     if tel_i == 1 %can't send output to function before output is defined
        %         %X = zeros(opt.tel_max, opt.bf.j*opt.bf.k*opt.bf.l*opt.bf.m*opt.bf.n*((2^(opt.tel_max-1))^opt.pd));
        ...
        parfor (i = 1:sim_length,opt.bf.maxworkers)
            if sim_run(i) == 1 %only run points where the sim_run variable is true (used for persistence opt)
                ...
                %disp(i)
                GenCoord = [Kwi(i),Ki(i),Kwa(i),Kd(i),Kc(i)]
                [C_temp(i),S_temp(i)] = ...
                    simHybrid(GenCoord, S(i),opt,data,atmo,batt,econ,uc,bc,dies,inso,wave,turb,cturb);
        ...
        elseif opt.tar == 2
            ...
            opt.min_gi = find(C_tot == min(C_tot))
            I_min(tel_i) = temp_I(opt.min_gi(1));
        end
...
%% GET OUTPUT VALUES
MinGenCoords = [output.min.kWwi{end}, output.min.kWi{end}, output.min.kWwa{end}, output.min.kWd{end}, output.min.kWc{end}];

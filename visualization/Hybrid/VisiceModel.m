...
grid on; box on
lg = legend('','Ice threshold','Interpreter','Latex','FontSize',fs);
...
hold on
plot(datetime(time,'convertfrom','datenum'),PwindNI/1000,'linewidth',1.2,'color','k','DisplayName','Wind')
...
text(0.05, 0.85,'\textbf{(c)}','Units','Normalized', ...
    ...
    'FontSize',fs,'Interpreter','latex')
grid on; box on
lg = legend('No Ice Power','','Interpreter','Latex','FontSize',fs);
lg.ItemTokenSize(1)= 10;
lg.Layout.Tile = 'north';
xlim([sttime,edtime])
xticks([])
...
hold on
plot(datetime(time,'convertfrom','datenum'),PinsoNI/1000,'linewidth',1.2,'color','k','DisplayName','Solar')

...
filename = dir(strcat(folder, filename));
if max(size(filename)) == 1
    xn = split(filename(1).name,"_");
    temp = load(fullfile(folder,filename(1).name));
    fn = fieldnames(temp);
    optStruct6D = temp.(fn{1});
else %This means the results are in a sensitivity structure
    for fi = 1:max(size(filename))
        xn = split(filename(fi).name,"_");
        temp = load(fullfile(folder,filename(fi).name));
        fn = fieldnames(temp);
...
for lcind = 1:3
    ...
    if lcind == 2
        ...
        legend(sgleg, "Hybrid",'location','northoutside','NumColumns',2)
    end
end

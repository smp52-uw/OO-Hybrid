function [] = visIWWCObjSpace(optStruct)
...
for it = 1:it_max(2)
    ...
    %create grid
    %[Kd,Ki,Kwi,Kwa,S]
    [Kigrid,Kwigrid,Kwagrid,Kcugrid,Sgrid]= ndgrid(kW1{it},kW2{it},kW3{it},kW4{it},Smax{it});
    Kculist = reshape(Kcugrid,[j*k*l*m*n 1]);
...
% %     cdata = reshape(a_sat,[j,k,l]);
    ...
    for a = 1:j
        for b = 1:k
            for c = 1:n
                %adjust kcu value to min for that iteration
                cur_val = output.min.kWc{it};
                x = X(a,b,c);
                y = Y(a,b,c);
                z = Z(a,b,c);
                s_ind = find(Kilist == x & Kwilist == y & Slist == z & Kwalist == 0 & Kculist == cur_val);

...
ax1 = nexttile;
scatter(x4,Ki(IS),18,col(5,:),'filled')
...
ax2 = nexttile;
scatter(x4,Kwi(IS),18,col(9,:),'filled')
%caxis([a_min4 ceil(max(p_max_mult4)*a_min4)])
...
ax3 = nexttile;
scatter(x4,Kwa(IS),18,col(10,:),'filled')
%caxis([a_min4 ceil(max(p_max_mult4)*a_min4)])
...
if opt.pm == 5
    ax4 = nexttile;
    scatter(x4,Kd(IS),5,col(1,:),'filled')
    %caxis([a_min4 ceil(max(p_max_mult4)*a_min4)])
...
elseif opt.pm == 4
    ax4 = nexttile;
    scatter(x4,Kc(IS),18,col(11,:),'filled')
    %caxis([a_min4 ceil(max(p_max_mult4)*a_min4)])
...
ax5 = nexttile;
scatter(x4,tot_G(IS),18,col2(7,:),'filled')
%caxis([a_min4 ceil(max(p_max_mult4)*a_min4)])
...
scatter(x4,Smax(IS),18,col(1,:),'filled')
%caxis([a_min4 ceil(max(p_max_mult4)*a_min4)])
...
ax7 = nexttile;
scatter(x4,cost_a(IS),18,col2(7,:),'filled')
%caxis([a_min4 ceil(max(p_max_mult4)*a_min4)])
...
ax8 = nexttile;
scatter(x4,surv(IS).*100,18,col2(7,:),'filled')
%caxis([a_min4 ceil(max(p_max_mult4)*a_min4)])
ylabel({'$a_{\mbox{sim}}$','[\%]'},'FontSize',fs,'interpreter','latex');
...
text(1.02, 0.5,'(h)','Units','Normalized', ...
    'VerticalAlignment','middle','FontWeight','normal', ...
    'FontSize',fs3,'Interpreter','latex')
set(gca,'FontSize',6)
%ylim([min(cost_a{5}(IS{5})) max(cost_a{4}(IS{4}))])

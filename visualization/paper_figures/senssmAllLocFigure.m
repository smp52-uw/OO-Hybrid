...
%         ax(a) = subplot(10,4,floor(a/4.1)*4+a+[0 4]);
        ax(a) = subplot(4,4,a);
        for i = 1:5
...
%         if f == 1
%             ylim([0 max(squeeze(max(cost(:,:,1:5),[],1:2))./t0(l(1:5))')])
%         else
%             ylim([0 max(cost(:,:,6:end)./t0(l(6:end)))])
%         end
%         yticks([0 1])
%         yticklabels({'$0k','Total Cost'})
        ylim([0 max(cost(:))])
        yticks([0 500000])
        ...
        if a == 2
            t = title('Instrumentation','FontWeight','normal', ...
                'FontSize',fs3);
            t.Position(2) = t.Position(2)*1.1;
        end
        if a == 3
